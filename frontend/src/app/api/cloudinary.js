...
export async function uploadPdf(file, id, user_type, options = {}) {
  try {
    const formData = new FormData();
    formData.append("file", file);
    formData.append("id", id);
    ...
    const response = await axios.post("/api/cloudinary/pdf/upload", formData, {
      headers: {
        "Content-Type": "multipart/form-data",
      },
    });
...
export async function deletePdf(id, user_type) {
  try {
    const formData = new FormData();
    formData.append("id", id);
    formData.append("user_type", user_type);

    const response = await axios.post("/api/cloudinary/pdf/delete", formData);
    return response.data;
  } catch (error) {
    console.error("Error deleting from Cloudinary:", error);
    throw error;
