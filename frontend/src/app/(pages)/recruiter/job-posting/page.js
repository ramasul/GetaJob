...
export default function PostingJob() {
    ...
    const fetchRecruiterData = async () => {
      ...
      try {
        ...
        const response = await fetch(
          `https://unconscious-puma-universitas-gadjah-mada-f822e818.koyeb.app/recruiters/${user.id}`
        );
        ...
        setError(
          "Failed to load profile data. Some fields may need to be filled manually."
        );
      } finally {
        setLoading(false);
      }
  ...
  const handleAddSkill = () => {
    if (
      newSkill.trim() !== "" &&
      !formData.required_skills.includes(newSkill)
    ) {
      setFormData({
        ...formData,
  ...
  return (
    <ProtectedRoute userType="recruiter">
      <div className="min-h-screen w-full bg-gradient-to-tr from-[#45D1DD] to-blue-300 pb-10">
        <Header currentPage="job-posting" userType={user?.user_type} />
              ...
              <div className="mb-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {formData.required_skills.map((skill, index) => (
                    <div
                      key={index}
                      className="bg-blue-100 text-gray-700 px-3 py-1 rounded-full flex items-center"
                    >
                      {skill}
                      <button
                        type="button"
